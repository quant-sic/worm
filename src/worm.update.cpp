...
int worm::INSERTWORM()
{
  ...
  if (operator_string[isite].size() != 1) // dummy element always there
  {
    ...
    if (prev_it == operator_string[isite].begin())
      prev_it = operator_string[isite].end();
    --prev_it;
...
#ifdef UNISYS
  if (!range_check(n_mid))
    return impossible;
#else
  if (MyModel->range_fail(n_mid))
    return impossible;
#endif
...
int worm::MOVEWORM()
{
  ...
  ++itn;
  if (itn == operator_string[isite].end())
    itn = operator_string[isite].begin();
  Diagram_type::iterator itp = worm_head_it;
  if (itp == operator_string[isite].begin())
    itp = operator_string[isite].end();
  --itp;
  ...
  if (dir == +1)
  {
    if (worm_at_stop == -1)
    {
      ...
      {
        if (itn->color() < 0)
          return impossible; // cannot pass worm tail
        ...
        for (size_t const &j : zc[isite])
        {
          if (nb[isite][j] == worm_tail_it->link())
          {
            ...
            ++it_up;
            if (it_up == operator_string[worm_tail_it->link()].end())
              it_up = operator_string[worm_tail_it->link()].begin();
      ...
      else if (worm_passes_nb_kink)
      {
        ...
        for (auto const &zci : zc[worm_head_it->link()])
        {
          ...
          it_up = it_nb;
          ++it_up;
          if (it_up == operator_string[adj_site].end())
            it_up = operator_string[adj_site].begin();
    ...
    if (worm_meas_densmat)
      t_loc = itn->time(); // exceptional case
    nbtime[zcmax] = t_loc;
    ...
    size_t nb_next = zcmax;
    double dt_min = dtime[zcmax];
    ...
    for (auto const &i : zc[isite])
      cout << "# MOVEWORM dir == +1 nb, dtime, nbtime " << i << "\t" << dtime[i] << "\t" << nbtime[i] << "\n";
    ...
    if (Metropolis(ratio))
    {
      ...
      {
#ifdef CAN_WINDOW
        if (abs((Nprtcls + (n_L - n_R) * tau) / beta - canonical) >= can_window)
          return impossible;
#endif
      ...
      else
      {
        ...
        if (t_w > beta)
          t_w -= beta;
#ifdef CAN_WINDOW
        if (abs((Nprtcls + (n_L - n_R) * pexp) / beta - canonical) >= can_window)
          return impossible;
#endif
        worm_head_it->time(t_w);
        worm_dtime += pexp;
  ...
  { // dir == -1
    if (worm_at_stop == +1)
    {
      ...
      if (worm_meas_densmat)
      {
        ...
        ++it_up;
        if (it_up == operator_string[isite].end())
          it_up = operator_string[isite].begin();
        for (size_t const &j : zc[isite])
        {
          if (nb[isite][j] == worm_tail_it->link())
          {
      ...
      else if (worm_passes_nb_kink)
      {
        ...
        for (auto const &zci : zc[isite])
        {
          ...
          it_nb = worm_head_it->get_assoc(zci);
          if (it_nb == operator_string[adj_site].begin())
            it_nb = operator_string[adj_site].end();
          --it_nb;
          size_t oppdir = opposite_direction[worm_head_it->link()][zci];
          if (!is_not_close(it_nb->time(), worm_head_it->time(), 2 * tol))
          {
    ...
    if (worm_meas_densmat)
      t_loc = itp->time(); // exceptional case
    nbtime[zcmax] = t_loc;
    ...
    size_t nb_next = zcmax;
    double dt_min = dtime[zcmax];
    if (t_between(worm_tail_it->time(), tmax, worm_head_it->time()))
    { // for density matrix
      tmax = worm_tail_it->time();
    ...
    if (Metropolis(ratio))
    {
      ...
      {
#ifdef CAN_WINDOW
        if (abs((Nprtcls + (n_R - n_L) * tau) / beta - canonical) >= can_window)
          return impossible;
#endif
        double t_w = tmax;
        if (tmax == beta)
      ...
      else
      {
        ...
        if (t_w < 0)
          t_w += beta;
#ifdef CAN_WINDOW
        if (abs((Nprtcls + (n_R - n_L) * pexp) / beta - canonical) >= can_window)
          return impossible;
#endif
        worm_head_it->time(t_w);
        worm_dtime -= pexp;
...
int worm::INSERTKINK()
{
...
#ifdef DEBUGMODE
  if (worm_diag)
    cerr << "Diagonal configuration in INSERTKINK ? \n\n";
#endif
  if (worm_at_stop != 0)
    return impossible;
  if (worm_meas_densmat)
    return impossible;
  if (worm_passes_nb_kink == 1)
    return impossible;
  for (size_t const &j : zc[worm_head_it->link()])
  {
    if (!is_not_close(worm_head_it->time(), worm_head_it->get_assoc(j)->time(), 2 * dtol))
      return impossible;
  }
  ...
  if (itp_adj == operator_string[adj_site].begin())
    itp_adj = operator_string[adj_site].end();
  --itp_adj;
...
#ifdef UNISYS
  if (!range_check(nbs_mid))
    return impossible;
#else
  if (MyModel->range_fail(nbs_mid))
    return impossible;
#endif
...
{
  if (worm_meas_densmat)
    return impossible; // no density matrix measurement
  ...
  if (worm_at_stop == +1 && worm_passes_nb_kink == 0)
  { // ie, the worm is just above the kink
    ...
    n_mid = it->after();
#endif
    if (n_out != worm_head_it->after())
      return (PASSINTERACTION(-1, it)); // this is a situation where n_out = 1, n_mid = 2, and the occup after the worm is 3.
  }
  else if (worm_at_stop == -1 && worm_passes_nb_kink == 0)
  { // dir==+1           // ie, the worm is just below the kink
    ++it;
    if (it == operator_string[isite].end())
      it = operator_string[isite].begin();
    if (it->color() != +1)
      return impossible;
#ifdef DEBUGMODE
    n_mid = it->before();
  ...
  cout << "DELETEKINK old weight, new weight : "
...
int worm::GLUEWORM()
{
  ...
  if (worm_diag)
    cerr << "Diagonal configuration in GLUEWORM ? \n\n";
#endif
  ...
  {
    ++it_upper;
    if (it_upper == operator_string[isite].end())
      it_upper = operator_string[isite].begin();
    if (!(it_upper == worm_tail_it))
      return impossible;
  }
  else
  {
    if (it_lower == operator_string[isite].begin())
      it_lower = operator_string[isite].end();
    --it_lower;
    if (!(it_lower == worm_tail_it))
      return impossible;
  }
