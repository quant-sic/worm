...
public:
  ...
  void initialize(string h5_filename);
  void test_conf();
...
inline void worm::get_diaginfo_nb_pos(const SiteIndex s)
{
  ...
  for (size_t const &i : zc[s])
  {
    ...
    itp = its;
    if (itp == operator_string[snb].begin())
      itp = operator_string[snb].end();
    --itp;
    if (worm_head_it->time() == its->time())
    {
      ++its;
      if (its == operator_string[snb].end())
        its = operator_string[snb].begin();
