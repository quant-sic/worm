...
void worm::define_parameters(parameters_type &parameters)
{
  ...
  alps::define_convenience_parameters(parameters)
      ...
      .define<string>("site_arrays", "sites.h5", "file with site arrays");
...
// #endif
  ...
  for (size_t i = 0; i < hist_gt.size(); i++)
    hist_gt[i] = 0;
#endif
...
// cannot be removed for non UNISYS -> results in segfault
#ifdef UNISYS
...
void worm::initialize_update_prob(update_tag begin, update_tag end)
{
  ...
  for (size_t upd = begin; upd < end; ++upd)
  {
    if (parameters.defined("p_" + update_names[upd]))
    {
      ...
      {
        throw std::runtime_error("Negative update probability: " + update_names[upd]);
      }
    }
    else
    {
...
void worm::initialize(string h5_filename)
{
  worm_diag = true;
  worm_at_stop = 0;
...
// #endif
  ...
  {
    for (size_t j = 0; j < update_tag::update_count; j++)
...
void worm::measure_corrfun()
{
  ...
  if (!parameters["pbcx"])
    return;
  if (LATTICE::dim > 1 && !parameters["pbcy"])
    return;
...
void worm::measure_Gpt()
{
  ...
  if (index >= hist_gt.size())
    std::runtime_error("index in measure_gpt is out of bounds ");
...
void worm::test_conf()
{
  for (SiteIndex i = 0; i < Nsites; i++)
  {
    for (Diagram_type::iterator it = operator_string[i].begin(); it != operator_string[i].end(); ++it)
    {
      ...
      ++itn;
      if (itn == operator_string[i].end())
        itn = operator_string[i].begin();
      ...
      if (it->color() == 1)
      {
        ...
        {
          if (b && abs(it->time() - it->get_assoc(j)->time()) < 1e-16 && it->get_assoc(j)->color() == 1)
            b2 = true;
          if (abs(it->time() - it->get_assoc(j)->time()) < 1e-16 && it->get_assoc(j)->color() == 1)
            b = true;
      ...
      if (it->color() != 0)
      { // extensive check on associations
        for (size_t const &j : zc[i])
        {
          ...
          if (itp == operator_string[adj_site].begin())
            itp = operator_string[adj_site].end();
          --itp;
  ...
  if ((worm_at_stop == +1) && (!worm_passes_nb_kink) && (!worm_meas_densmat))
  {
    ...
    if (itp == operator_string[worm_head_it->link()].begin())
      itp = operator_string[worm_head_it->link()].end();
    --itp;
  ...
  if ((worm_at_stop == -1) && (!worm_passes_nb_kink) && (!worm_meas_densmat))
  {
    ...
    ++itn;
    if (itn == operator_string[worm_head_it->link()].end())
      itn = operator_string[worm_head_it->link()].begin();
  ...
  if (worm_passes_nb_kink)
  {
    ...
    for (size_t const &j : zc[worm_head_it->link()])
    {
      ...
      for (Diagram_type::iterator it = operator_string[adj_site].begin(); it != operator_string[adj_site].end(); ++it)
        if (it->time() == worm_head_it->time())
          b = true;
    }
    if (!b)
    {
...
double worm::calc_potential_energy_loc()
{
  ...
  for (int isite = 0; isite < Nsites; isite++)
  {
    ...
    for (;;)
    {
      ...
      t_old = it1->time();
      ++it1;
      if (it1 == operator_string[isite].end())
        break;
    }
  }
  return (En / beta);
  ...
  for (int isite = 0; isite < Nsites; isite++)
  {
    for (auto const &iz : zc[isite])
    {
      ...
      for (;;)
      {
        for (;;)
        {
          if (it2 == operator_string[jsite].end())
            break;
          if (it2->time() > it1->time())
            break;
          dt = it2->time() - t_old;
#ifdef UNISYS
...
void worm::find_assoc_insert(const SiteIndex cursite, Diagram_type::iterator it, const int shift)
{
  ...
  ito = it;
  ++ito;
  if (ito == operator_string[cursite].end())
    ito = operator_string[cursite].begin();
  ...
  for (size_t j = 0; j < zcmax; ++j)
  {
    ...
    itl = ito->get_assoc(j);
    itp = itl;
    if (itp == operator_string[s].begin())
      itp = operator_string[s].end();
    --itp;
    it->set_assoc(j, itl);
...
void worm::find_assoc_delete(const SiteIndex cursite, Diagram_type::iterator it)
{
  ...
  Diagram_type::iterator itn = it;
  ++itn;
  if (itn == operator_string[cursite].end())
    itn = operator_string[cursite].begin();
  Diagram_type::iterator itp = it;
  if (itp == operator_string[cursite].begin())
    itp = operator_string[cursite].end();
  --itp;
    ...
    else
    {
      while (itt != it_end)
      {
        if (itt->get_assoc(oppdir) == it)
          itt->set_assoc(oppdir, itn);
        ++itt;
        if (itt == operator_string[s].end())
          itt = operator_string[s].begin();
      }
      if (it_end->get_assoc(oppdir) == it)
        it_end->set_assoc(oppdir, itn);
    } // else
  }   // for
}
