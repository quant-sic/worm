...
public:
...
#ifdef UNISYS
        ,
        U_on(params["U_on"]), V_nn(params["V_nn"]), mu(params["mu"]), t_hop(params["t_hop"])
#endif
  {
    nmin = 0;
  ...
  {
    if (range_fail(n1) || range_fail(n2))
      return 0;
    if (n1 == n2 + 1)
    {
      return sqrt(n1 * 1.);
  ...
  {
    if (range_fail(n1) || range_fail(n2) || range_fail(m1) || range_fail(m2))
      return 0;
    double w = 0.;
    if ((n1 == n2 + 1) && (m2 == m1 + 1))
    {
  ...
  void print_params(std::ostream &os) const override
  {
    ...
    os << "# model : U_on                                      : ";
    for (size_t i = 0; i < U_on.size(); ++i)
      os << U_on[i] << " ";
    os << "\n";
    os << "# model : V_nn                                      : ";
    for (size_t i = 0; i < V_nn.size(); ++i)
      os << V_nn[i] << " ";
    os << "\n";
    os << "# model : mu                                        : ";
...
public:
...
#ifdef UNISYS
        ,
        Jpm(params["Jpm"]), Jzz(params["Jzz"]), h(params["h"])
#endif
  {
    nmin = 0;
    nmax = params["nmax"].as<StateType>();
  ...
  double site_weight_offdiag(const SiteIndex s, const StateType n1, const StateType n2) const override
  {
    if (range_fail(n1) || range_fail(n2))
      return 0;
    if (n2 == n1 + 1)
      return sqrt(Sspin * (Sspin + 1.) - stateval(n1) * stateval(n2));
    if (n1 == n2 + 1)
      return sqrt(Sspin * (Sspin + 1.) - stateval(n1) * stateval(n2));
    return 0;
  }
  ...
  void print_params(std::ostream &os) const override
  {
    ...
    os << "# User specified inhomogeneous parameters:\n";
    os << "# model : Jpm                                      : ";
    for (size_t i = 0; i < Jpm.size(); ++i)
      os << Jpm[i] << " ";
    os << "\n";
    os << "# model : Jzz                                      : ";
    for (size_t i = 0; i < Jzz.size(); ++i)
      os << Jzz[i] << " ";
    os << "\n";
    os << "# model : h                                        : ";
