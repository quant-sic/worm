...
void worm::print_params(std::ostream& os) const {
  ...
  os << "# Lattice basis                                     : " << parameters["basis_name"].as<std::string>() << "\n";
  os << "# Lattice cell                                      : " << parameters["cell_name"].as<std::string>() << "\n";

  //Lattice.print(os); //for detailed output
...
void worm::save(alps::hdf5::archive & ar) const {
  ...
  // std::vector<double> wind_vec(mWinding.data(), mWinding.data() + mWinding.size());  // XXX  winding number deactivated
  ...
  // ar["checkpoint/configuration/mWinding"] << wind_vec;  // XXX winding number deactivated
...
void worm::load(alps::hdf5::archive & ar) {
  ...
  for (auto const& row : op_string_arr){
    ...
    while (col_it != end) {
      ...
      vertexlist.insert(vertexlist.end(), Element(before, after, link, time, color, Lattice.num_neighbors(link)));
      col_it += zcmax;
    }
  ...
  for (SiteIndex i = 0; i < Nsites; i++) {
    for (size_t j = 0; j < Lattice.num_neighbors(i); j++) {
      dummy_it[i]->set_assoc(j, dummy_it[Lattice.neighbor(i,j)]);
    }
  }
  ...
  for (SiteIndex i = 0; i < Nsites; i++) {
    ...
    for (auto elem_it = operator_string[i].begin(); elem_it != operator_string[i].end(); ++elem_it, ++elem_idx) {
      ...
      for (size_t j = 0; j < Lattice.num_neighbors(i); j++) {
          auto assoc_elem_it = find_elem(operator_string[Lattice.neighbor(i,j)].begin(), 
                                         operator_string[Lattice.neighbor(i,j)].end(), assoc_vec.at(j));
          if (assoc_elem_it != operator_string[Lattice.neighbor(i,j)].end())
  ...
  // mWinding = Eigen::Map<LATTICE::Direction>(wind_vec.data()); // XXX winding number deactivated
